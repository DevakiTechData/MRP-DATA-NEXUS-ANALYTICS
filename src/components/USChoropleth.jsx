...
const USChoropleth = ({ data = {}, title = 'Alumni Distribution' }) => {
  const states = Object.keys(data).filter(Boolean);
  const values = states.map((code) => data[code]);

  if (states.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-sm text-gray-500">
        <p>No alumni state data available.</p>
      </div>
    );
  }
  ...
  const plotData = [
    {
      ...
      hovertemplate:
        '<b>%{location}</b><br>Alumni: %{z}<extra></extra>',
    },
    {
      type: 'scattergeo',
      locationmode: 'USA-states',
      locations: states,
      text: states.map((code) => STATE_NAMES[code] || code),
      mode: 'text',
      textfont: {
